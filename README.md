...
系統不直接產生最終 `.pptx`，也不讓 AI 重新設計實驗室模板。正確流程是：AI 先依 Prompt 產生內容頁，再將內容頁複製或套用至實驗室既有 PowerPoint 模板。
...
3. Prompt 中列出的所有研究圖片。
4. 實驗室 PowerPoint 模板或優良簡報參考檔。

可以另外告訴 AI：

```text
請嚴格依照 final_prompt.txt 製作內容頁，只能使用我提供的研究資料與圖片。
不要重新生成封面，也不要重新設計 NanoSTLab 模板的母片、Logo、頁首頁尾及頁碼。
完成內容頁後，請套用或複製到我提供的實驗室 PowerPoint 模板。
```
