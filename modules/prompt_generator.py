...
def generate_final_prompt(user_inputs: dict, style_rules: dict, analysis: dict) -> str:
    ...
    lines.extend([
        ...
        "4. 一般正文、說明句、項目內容、圖說與引用必須使用 Regular，不得把整個文字框、整段或整頁設為粗體。每張結果頁最多只將一個短結論關鍵詞或關鍵數值加粗，正文粗體字數不得超過約 15%；必要時再使用技術藍或警示紅。",
...
def _visual_rules_summary(style_rules: dict) -> list[str]:
    ...
    return [
        ...
        f"- 【字重硬規則】{typography.get('bold_scope', '一般正文、說明句、項目內容、圖說與引用一律使用 Regular；粗體只用於投影片標題、短 callout 標頭，以及每張結果頁最多一個短結論關鍵詞或關鍵數值。不得把整個文字框、整段或整頁設成粗體。')} 正文粗體可見字數必須低於約 15%。",
        ...
        "- 結果頁最多只加粗一個短結論關鍵詞或關鍵數值；其餘正文保持 Regular。技術機制可用藍色，負向漂移、劣化或風險才用紅色。不得整段全粗或整頁同色。",
...
def _design_instructions(section: str, has_uploaded_image: bool) -> str:
    ...
    if section == "results":
        return base + "所有說明文字使用 Regular；最多只將一個短結論關鍵詞或關鍵數值加粗，粗體不得超過正文約 15%。有比較或多圖順序時加入原生箭頭。只有能明確指出局部 ROI 時才用一個透明細虛線框緊貼該圖區，否則不要加框。"
    return base + "所有正文使用 Regular；只有確實需要強調時，才可將一個短關鍵詞或關鍵數值加粗。只有技術機制或負向風險需要時才分別使用藍色或紅色。"
